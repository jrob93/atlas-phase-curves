...
def phase_fit_func_mpc(mpc_number,end_date):
    with open("tmp", "w") as f:
        ...
        fit = sbpy_phase_fit_app.phase_fit(mpc_number=mpc_number,end_date=end_date,**function_flags)
        check=fit.calculate()
...
def phase_fit_func_name(name,end_date):
    with open("tmp", "w") as f:
        sys.stdout = f
        fit = sbpy_phase_fit_app.phase_fit(name=name,end_date=end_date,**function_flags)
        check=fit.calculate()
        sys.stdout = sys.__stdout__
    return check

# wipe the log file
log_file="sbpy_phase_fit_app.log"
...
object_lists=[mpc_number_list[:10],name_list[:10]]
...
fname="fit_phase_curves_app_bulk_record_{}-{}-{}.csv".format(today.day,today.month,today.year)
print(fname)
of=open(fname,"w")
of.write("date,mpc1,mpc2,t(s),N_jobs,rate(1/s)\n")

# LOG ALL SETTINGS HERE
fit = sbpy_phase_fit_app.phase_fit(mpc_number=1,name="Ceres",end_date=end_date_mjd,**function_flags)
...
cmd="mv sbpy_phase_fit_app.log sbpy_phase_fit_app_{}-{}-{}.log".format(today.day,today.month,today.year)
